...
Table initializeTable(int NumOfVertex, Vertex startV) {
	...
	if (T != NULL) {
		...
		T->isInTheQueue = (boolean *)malloc(sizeof(boolean) * (NumOfVertex + 1));
		T->path = (int *)malloc(sizeof(int) * (NumOfVertex + 1));
		T->dequeueCounter = (int *)malloc(sizeof(int) * (NumOfVertex + 1));

		for (i = 1;i <= NumOfVertex;i++) {
			T->dist[i] = Infinity;
			T->isInTheQueue[i] = false;
			T->path[i] = 0;
			T->dequeueCounter[i] = 0;
		}
		T->dist[startV] = 0;
	}
...
void displayTable(Table T) {
	...
	printf_s(" v\tisInTheQueue\tcounter\tdist\tpath\n");
	for (i = 1;i <= T->NumOfVertex;i++)
		printf_s(" %d\t%d\t\t%d\t%d\t%d\n", i, T->isInTheQueue[i], T->dequeueCounter[i], T->dist[i], T->path[i]);
}
